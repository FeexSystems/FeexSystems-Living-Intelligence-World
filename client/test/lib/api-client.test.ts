...
import { apiClient, isApiError, handleApiError, ApiError } from '@/lib/api-client';
...
describe('API Client', () => {
  ...
  describe('2.2.1 - Successful Request', () => {
    it('should make successful request', async () => {
      ...
      expect((response as any).projects).toBeDefined();
      expect(Array.isArray((response as any).projects)).toBe(true);
    });
    ...
    it('should return correct response structure', async () => {
      ...
      expect((response as any).edges).toBeDefined();
      expect(Array.isArray((response as any).nodes)).toBe(true);
      expect(Array.isArray((response as any).edges)).toBe(true);
    });
  });
