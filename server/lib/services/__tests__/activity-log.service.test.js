 function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import { describe, it, expect, beforeEach, afterEach } from 'vitest';
...
describe.skip('ActivityLogService', () => {
  ...
  describe('getMostActiveUsers', () => {
    ...
    it('should get most active users', async () => {
      ...
      expect(_optionalChain([result, 'access', _ => _[0], 'access', _2 => _2.user, 'optionalAccess', _3 => _3.id])).toBe(testUser.id);
  ...
  describe('getActivityStats', () => {
    ...
    it('should get activity statistics', async () => {
      ...
      expect(_optionalChain([profileViewedStat, 'optionalAccess', _4 => _4.count])).toBe(2);

      // Check resource stats
      const userProfileStat = stats.resourceStats.find(s => s.resource === 'USER_PROFILE');
      expect(_optionalChain([userProfileStat, 'optionalAccess', _5 => _5.count])).toBe(3);
    });
