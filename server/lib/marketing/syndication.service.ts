...
export interface SyndicationTask {
  assetId: string;
...
export class SyndicationService {
   ...
   */
  async generateDraft(assetId: string, platform: "twitter" | "linkedin" | "email"): Promise<SocialPostDraft> {
...
`;
   ...
   */
  async syndicateAsset(task: SyndicationTask): Promise<{ status: string; url?: string }> {
