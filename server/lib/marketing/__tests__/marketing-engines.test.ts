...
describe('Marketing Intelligence Engines', () => {
  ...
  describe('Decay Engine', () => {
    it('should flag assets with old events as decayed', async () => {
      ...
      (prisma.marketingContentAsset.findMany as any).mockResolvedValue([
        {
          id: 'asset-1',
          title: 'Old Asset',
  ...
  describe('Gap Engine', () => {
    it('should identify products with features but no assets as gaps', async () => {
      (prisma.marketingProduct.findMany as any).mockResolvedValue([
        {
          id: 'prod-1',
  ...
  describe('Opportunity Engine', () => {
    it('should detect opportunities from recent releases on products with gaps', async () => {
      (prisma.worldModelEvent.findMany as any).mockResolvedValue([
        {
          projectId: 'proj-1',
          eventType: 'release',
          project: {
    ...
    it('should create a recycling stub preserving lineage', async () => {
      (prisma.marketingContentAsset.findUnique as any).mockResolvedValue({
        id: 'parent-asset',
        title: 'Original Title',
        type: 'ARTICLE'
      });

      (prisma.marketingContentAsset.create as any).mockResolvedValue({
        id: 'new-stub'
      });
