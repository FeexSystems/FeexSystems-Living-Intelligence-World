...
function retrievalInstance(service: MarketingNavigatorService) {
  return (service as unknown as { retrieval: any }).retrieval;
}
...
describe("MarketingNavigatorService", () => {
  let service: MarketingNavigatorService;
  let retrieval: ReturnType<typeof retrievalInstance>;
  ...
  beforeEach(() => {
    ...
    service = new MarketingNavigatorService({} as any);
  ...
  describe("recommendations", () => {
    ...
    it("is capped at 10 recommendations", async () => {
      ...
      vi.mocked(detectContentGaps).mockResolvedValue(manyGaps as any);
  ...
  describe("exploreMarketingGraph (deprecated wrapper)", () => {
    it("delegates to buildContext and includes campaigns", async () => {
      ...
      const ctx = await (service as any).exploreMarketingGraph("vyra");
      expect(ctx.campaigns).toHaveLength(1);
